...
#| echo: false
df <- tibble(
  time = c(0, 1, 2, 4, 8, 16, 32),
  concentration = c(0, 8, 12, 14, 9, 4, 2)
)
```
...
```{r echo=FALSE}
ggplot(df, aes(time, concentration)) + 
  geom_line() +
  geom_point(size = 2) + 
...
| Quantity      | Notes          | Symbol             | Unit     | Formula        | Example  |
+===============+================+====================+==========+================+==========+
| Absorption    | The time taken | $t_{\frac{1}{2}a}$ | h        | $\ln(2) / k_a$ | 1.0 h    |
| half-life     | for 50% of the |                    |          |                |          |
|               | dose to be     |                    |          |                |          |
|               | absorbed into  |                    |          |                |          |
|               | circulation    |                    |          |                |          |
+---------------+----------------+--------------------+----------+----------------+----------+
| Elimination   | The time taken | $t_{\frac{1}{2}b}$ | h        | $\ln(2) / k_e$ | 12 h     |
| half-life     | for the drug   |                    |          |                |          |
|               | concentration  |                    |          |                |          |
|               | to fall to 50% |                    |          |                |          |
|               | of its initial |                    |          |                |          |
|               |value           |                    |          |                |          |
...
Of course, when an uninitiated reader looks at these formulas, the first question they'd ask is "what the heck do the $k_a$ and $k_e$ quantities refer to? In a slightly circular fashion, we can extend the table somewhat and refer to these as the corresponding rate parameters for absorption and elimination:

+---------------+----------------+--------------+----------+---------------------+--------------+
| Quantity      | Notes          | Symbol       | Unit     | Formula             | Example      |
+===============+================+==============+==========+=====================+==============+
| Absorption    | The rate at    | $k_a$        | $h^{-1}$ | $\frac{\ln 2}       | 0.693 h^-1^  |
| rate          | which the drug |              |          | {t_{\frac{1}{2}a}}$ |              |
|               | enters into    |              |          |                     |              |
|               | circulation    |              |          |                     |              |
+---------------+----------------+--------------+----------+---------------------+--------------+
| Elimination   | The rate at    | $k_e$        | $h^{-1}$ | $\frac{\ln 2}       | 0.0578 h^-1^ |
| rate          | which the drug |              |          | {t_{\frac{1}{2}b}}  |              |
|               | is eliminated  |              |          | = \frac{CL}{V_d}$   |              |
|               | from the body  |              |          |                     |              |
+---------------+----------------+--------------+----------+---------------------+--------------+
...
Once we have the concept of a clearance rate $CL$ -- and implicitly, a concept of how it relates to elimination rates $k_e$ and distribution volume $V_d$ -- we can start linking it to observable (or at least, estimable) quantities...
...
```{r echo=FALSE}
ggplot(df, aes(time, log(concentration))) + 
  geom_line(colour = "grey50", linetype = "dotted") +
  geom_point(size = 2) + 
  geom_smooth(data = df[4:7,], method = "lm", formula = y ~ x, se = FALSE) +
  theme(panel.grid = element_blank())
```

Expressed as R code, we can estimate the elimination rate by regressing log concentration against time for the final few data points (about 3 or 4 is considered okay) and using the slope^[Well, -1 times the slope, but whatever.] as our estimate:
...
Suppose, for example, that instead of a one-compartment bolus model, the pharmacokinetics of our drug are better described by a "Bateman" curve that assumes a constant rate of absorption and a constant rate of elimination: 
...
```{r, echo=FALSE}
bateman <- function(time, 
                    ka, 
                    ke, 
                    bioavailability = .8, 
                    dose = 200, 
                    volume = 6) {
  const <- bioavailability * dose  * ka / volume / (ka - ke)
  const * (exp(-ke * time) - exp(-ka * time))
}

t <- seq(0, 50, .1)
df1 <- data.frame(
  time = t,
  concentration = bateman(t, ka = .25, ke = .15)
)

ggplot(df1, aes(time, concentration)) + 
  geom_line() +
  theme(panel.grid = element_blank())
```
