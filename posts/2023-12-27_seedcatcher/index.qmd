...
Well that was long and tiresome. I seem to be pathologically incapable of writing a short blog post without going off on bizarre yak-shaving tangents. Sorry. Anyway, let's get back on track and do something that relies on the state of the RNG, shall we? First, we'll reset the value of `.Random.seed` and capture its initial value:
...
Yep, same answer. This is what we should expect, because `x` and `y` were both created to be identical generators, and they were both designed to be isolated from the global RNG state. And so, with the tedious narrative inevitability of a thing written up in a blog post, it turns out that I get the exact same answer the first time I use generator `x` and the first time I use generator `y`, even though the global RNG state is different at the time `x` and `y` are invoked.
