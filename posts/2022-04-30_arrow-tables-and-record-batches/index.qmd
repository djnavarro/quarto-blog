...
This works and now we have a data frame

```{r}
florence |> 
  select(where(~!is.list(.))) |>  # drop list columns
  as_arrow_table() |>             # convert to arrow table
...
This is a regular arrow table:

```{r}
danceability$track_name
```

Can convert to tibble:
