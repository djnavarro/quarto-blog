...
Tricky thing is how to talk about chunked arrays without first talking about record batches, and the concept of sequentially arriving record batches. I think the way around this is to ask people to imagine multiple arrays arriving from different sources (with the promissory note that this will be explained later) and wanting to treat them as "just one thing" without shuffling anything around in memory or creating a new object (remind them arrays are immutable: R users will find that hard). Think about having a wrapper that points to the different arrays: each array is now a "chunk" in some new structure... etc.
