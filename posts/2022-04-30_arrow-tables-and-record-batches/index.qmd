...
Okay, so scalars aren't very interesting for the purposes of this post. Let's turn our attention to arrays next. Broadly speaking, an array is roughly analogous to a vector in R, and the **arrow** package is written in a way that allows you to interact with Arrow arrays in ways that feel very familiar to R users. I can create an array using `Array$create()`,
...
As a reminder, here's a crude schematic diagram showing how that object is laid out. It has some metadata that you've already learned how to extract, and it has two data buffers that I talked about at tedious length but haven't actually *shown* you yet:
