...
Excellen. Next, I'll use `separate()` again to split the `years` interval into two columns, one for the `year_started` and another for the `year_ended`. Having done so, the year information is finally in a numeric format, so I can coerce it from character to integer:
