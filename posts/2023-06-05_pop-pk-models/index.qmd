...
ids <- unique(warfpk$id)
t_obs <- purrr::map(ids, ~ warfpk$time[warfpk$id == .x & warfpk$mdv == 0])
c_obs <- purrr::map(ids, ~ warfpk$dv[warfpk$id == .x & warfpk$mdv == 0])
n_obs <- purrr::map_int(t_obs, length)
dose <- purrr::map_dbl(ids, ~ warfpk$amt[warfpk$id == .x & !is.na(warfpk$rate)])
