...
Yikes. Those R-hat values are a bit of a worry.
...
draws <- readr::read_csv("model1_draws.csv", show_col_types = FALSE) |>
  dplyr::select(tidyselect::matches("(theta|sigma|omega|lp)")) |> 
  dplyr::mutate(
    id = dplyr::row_number(),
    chain = ceiling(id/1000)
  )
draws
```
...
draws_long |> 
  dplyr::filter(chain %in% 1:2) |>
  ggplot(aes(value)) +
  geom_density(trim = TRUE, fill = "black") + 
  facet_grid(chain ~ parameter, scales = "free")

draws_long |> 
  dplyr::filter(chain %in% 3:4) |>
  ggplot(aes(value)) +
  geom_density(trim = TRUE, fill = "black") + 
  facet_grid(chain ~ parameter, scales = "free")
```
...
```{r}
#| label: posterior-lp-plots
ggplot(draws, aes(factor(chain), lp__)) + geom_violin() 
```


### Model fits

Under normal circumstances I could simply use the `$summary()` method to do all the work for me in extracting parameter estimates. However, because in this case I've decided that I only trust chains 3 and 4, I'm going to do it manually from the raw samples:
