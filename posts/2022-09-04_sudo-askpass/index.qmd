...
All of which is by way of preamble because I want to stress that when I talk about package dependencies in R there is no chance whatsoever that I will shy away from making the most of the queer double entendre opportunities that this topic provides.^[Like so many moments in life when I get this way, [Dan Simpson](https://twitter.com/dan_p_simpson/status/1565948834542669826) is at least partially to blame.] And so without further ado, today's unhinged rambling...
...
At the terminal, I can invoke ssh-askpass manually if I want to. It doesn't do much: all it does is create the dialog box and return the text input by the user.

``` bash
ssh-askpass
```

In practice you don't actually call this directly. What you do is configure sudo so that whenever it needs authentication but doesn't have access to a terminal, it will use ssh-askpass as the mechanism to ask permission. 

Here's the lines I added to sudo.conf:^[In the interests of strict accuracy, I actually had to create this file myself because it didn't already exist on my system: sudo was using all the default settings.] ^[As an aside, I had to use `whereis ssh-askpass` in order to find the path to ssh-askpass because the advice I'd seen online listed a different path and, shockingly, it doesn't work if you don't provide the correct path.]
...
## Can we relax a little sweetie?

The only problem is that quarto installation requires *five* system dependencies to be installed, and as the output below shows, pak creates a separate shell command for each one...
...
...and yes, I had to enter my password five times. 

That's mildly irritating, and I was not expecting it. My original assumption would be that entering the password the first time would invoke the sudo password cache: that is, after entering my password once, the elevated permissions^[Negotiations are always vital, babes, and elevated access permissions are not persistent. CNC isn't for newbies.] would persist for about 15 minutes. That's what happens by default at the terminal, and I had presumed the same would apply when the call to sudo occurs within an R session. However, that's not *quite* accurate. This little gem in `man sudo` explains the relevant security policy issue: 
...
The security policies in sudo are managed by a plugin system that is configured using the "sudoers" file(s). I'll talk about this one momentarily, but first here's the relevant extract from `man sudoers` that tells us what setting we need to modify: 
