...
The correlation here is `r cor(mmk_bolus_draws$km, mmk_bolus_draws$v0)`, pretty similar to the previous one.^[Looking at that plot I had wondered if the correlation was being boosted by a few outliers, but that doesn't seem to be the case: the rank-order correlation for the third plot is `r cor(mmk_bolus_draws$km, mmk_bolus_draws$v0, method = "spearman")`, which compares to `r cor(mmk_bolus_draws$k0, mmk_bolus_draws$v0, method = "spearman")` for the previous plot, and `r cor(mmk_bolus_draws$km, mmk_bolus_draws$vm, method = "spearman")` for the first one.] 
...
In this scenario the state $\mathbf{c}_t = (c_{gt}, c_{ct}, c_{pt})$ at time $t$ is defined by three concentrations:^[It should be noted that my notation is slightly nonstandard. My understanding is that the convention in the literature is to use notation consistent with the [WinNonlin](https://www.certara.com/software/phoenix-winnonlin/) software package, as that is generally considered the industry standard and often used for benchmarking purposes. I haven't done this for a very boring reason: I've not yet had a chance to play around with WinNonlin.]
...
- $k_a$ is the absorbtion rate (from gut to central compartment)
...
Second, we can define two clearance parameters:^[Yes, seriously. I know my notation is nonstandard. Hush. I'm still wrapping my head around it and I *like* having notation that makes clear that $q_e$ and $q_{cp}$ are both parameters of the same structural kind. I don't find CL and Q anywhere near as easy to understand.]
