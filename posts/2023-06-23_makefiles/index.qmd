...
Hope yet lives, despite our first descent accompanying Lord Morpheus of the Endless into hell. 
...
## Basic Makefiles, Act II: The kraken of the imagination is released
...
As before, our project has the following source files:
...
!["Me(dusa) too". Oil painting of the mythological character, Medusa, reimagined through a contemporary feminist lens, in response to the #metoo movement by [Judy Takács](https://en.wikipedia.org/wiki/Judy_Tak%C3%A1cs). Released by the artist as CC-BY.](img/medusa-too.jpg){fig-alt="Oil painting of the mythological character, Medusa, reimagined through a contemporary feminist lens"}

:::

## Basic Makefiles Act III: The tragedy of Medusa, and what is permitted to be seen or spoken
...
I'm also going to preface all my commands with the `@` symbol, which prevents them from being printed to the terminal whenever `make` is called. In place of the automatic printing, I'll use `echo` to write my own, more human-friendly output lines. So now my Makefile looks like this:
...
## Death of the Authoress: The fourth wall shatters into little shards of Recursion
