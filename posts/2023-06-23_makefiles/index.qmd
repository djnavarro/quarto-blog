...
This line in the code creates a variable called `src_files` corresponding to the following values:

```
src/collatz.cpp src/species.cpp src/swap.cpp
```

If we could be assured that the binary files always exist, we could use the same trick to list all binaries in the `bin` folder. But because those might not exist (e.g., if we delete the binaries when calling `make clean`), we can't be assured of that. So instead, we'll do a pattern substitution: we'll take the `src_files` variable as input, strip the `.cpp` extension from the files, and replace `src` with `bin`. 

``` Makefile
bin_files := $(patsubst src/%.cpp, bin/%, $(src_files))
```

The `bin_files` variable contains these values:

```
bin/collatz bin/species bin/swap
```

So now we have a variable (`bin_files`) listing the binary targets that our Makefile needs to create, as well as a variable (`src_file`) specifing the source files that need to be compiled in order to create those files. Now that we have this, we can set our "all" target as follows:
...
So our next task is to write a rule that will create targets for each of these binaries, and build them from the corresponding C++ source code:
