...
rate is so low (2.5%) the distribution of samples is highly irregular. Finally, in the middle panel, if we
...
about the burn-in issue. Suppose you started the sampler at a very bad location... say `x0 = -3`, and -- just so that we can exaggerate the problem -- we'll use a proposal distribution that is too narrow, say `sigma = .1`. The image below shows three runs of this sampler:
...
As you can see, the sampler spends the first 200 or so iterations slowly moving rightwards towards
...
between samples. The effect of this is illustrated in below, which shows the behaviour of a  sampler with a very wide proposal distribution (`sigma = 50`) with `n = 1000` samples drawn, at `lag = 0` (left), `lag = 10` (middle) and `lag = 100` (right).
