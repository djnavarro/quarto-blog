...
```{r, filename="[R code]", message=FALSE}
library(arrow)
library(reticulate)
use_miniconda("base")
```
...
Here we have a `put_table()` Python function does the same job that the `flight_put()` function does in the R example I presented earlier. It is a high-level wrapper function that sends a `do_put()` call to the server, streams the data across, then stops. The second line of code in this function is the one that makes the `do_put()` call:

```{python, eval=FALSE}
writer, reader = self.con.do_put(desc, table.schema)
```

Okay, so the object that I've called `desc` on the client side is the flight descriptor, and it's also the thing that later gets used on the server side to create the key against which the table is stored. If we look at the preceding line of code, we can see that the `desc` object is an instance of the `FlightDescriptor` class. So let's actually step into the Python console and run the commands required to create a flight descriptor object:
...
Looking at these two code extracts side by side we can see that the ticket object created by the client when `flight.Ticket()` is called ends up being used by the server to retrieve the requested table. So we should take a look at what happens here. What we hope to see is that this ticket ends up producing the same key that would have been used to store the data originally: that is, when the server uses `self.tables[ticket.ticket]` to retrieve data with the `do_get()` method, it should be producing the same answer that we got in the server-side `do_put()` method in the previous section that used `self.tables[descriptor.command]` to store the data. Let's verify that this is actually true!

Since I already have a `name` object lying around from earlier, let's run that line of code shall we?
