...
```{python imports, filename="tiny_flight.py", eval = TRUE}
```
...
```{python, filename="[server]", eval=FALSE}
...
```{python, filename="[client]", eval=FALSE}
class TinyClient:
...
There's a few things to comment on here. The `server_message()` function isn't very interesting for our purposes: it doesn't do anything other than print a message. The really important part of this code is the one line that does all the server-side work:

```{python, eval=FALSE}
self.tables[descriptor.command] = reader.read_all()
```

The `reader` object here that is passed as one of the arguments to `do_put()` is a [RecordBatchStreamReader](https://arrow.apache.org/docs/python/generated/pyarrow.ipc.RecordBatchStreamReader.html): it's an object capable of receiving a stream of Arrow data. When the `read_all()` method is called, it reads all record batches sent by the client and returns the final result as an Arrow table. This table is then stored in the `self.tables` dictionary. 

Notice that the key against which the table is stored as the value is specified by `descriptor.command`. This also needs to be unpacked! What is a "descriptor"? What is the "command" attribute of a descriptor? That's not obvious from inspection. To make sense of this, it helps to realise that this `descriptor` object is being transmitted to the `TinyServer` from the `TinyClient`. So let's look at the code I wrote for the client side:
...
This `put_table()` Python function does the same job that the `flight_put()` function is doing in R. It's a high-level wrapper that sends a `do_put()` call to the server, streams the data across, then stops. The line of code that makes the `do_put()` call is this one:

```{python, eval=FALSE}
writer, reader = self.con.do_put(desc, table.schema)
```

The `desc` object is the flight descriptor. This is the thing that is used on the server side to create the key against which the table is stored. The `desc` object is an instance of the `FlightDescriptor` class, created on the previous line. Let's have a look at what happens:
...
Perhaps unsurprisingly, the `command` attribute is in fact the (byte encoded) string that we used to specify the `name` of the data set. In other words, once we strip back all the layers here it turns out that the server stores the data set using the name that the client gave it!
...
Looking at these two code extracts side by side we can see that the ticket object created by the client when `flight.Ticket()` is called ends up being used by the server to retrieve the requested table. So we should take a look at what happens here. Since I already have a `name` object lying around from earlier, let's run that line of code shall we?
...
If we take a peek at the `ticket.ticket` object that gets used on the server side, we see that -- yet again -- under the hood the ticket is really just an alias for the name of the data set:
...
Notice that the server-side work is being done by the `collect_flight_info()` method. Here's the code for that one: 
