...
class TinyFlightServer(flight.FlightServerBase):
  
    # On initialisation, create a TinyFlightServer object specifying the host 
    ...
    def __init__(self, host = "localhost", port = 5005):
        ...
        # Initialise TinyFlightServer as an instance of 
        # flight.FlightServerBase using the appropriate location
        super(TinyFlightServer, self).__init__(location)
        
        # Set some initial values for the TinyFlightServer instance
        self.flights = {}
        self.host = host
    ...
    # do that we stream to a "mock" sink. No data is copied or retained at 
    ...
    def list_flights(self, context, criteria):
        ...
        for key, table in self.flights.items():
            descriptor = TinyFlightServer._get_descriptor(key)
            flight_info = self._get_flight_info(key, descriptor, table)
            yield flight_info
    ...
    def do_put(self, context, descriptor, reader, writer):
        print("do_put")
        key = TinyFlightServer._get_key(descriptor)
        print(key)
