...
Let's pop over into a different folder where I've stored a slightly fancier project:

``` bash
cd ../test-on-arch
```

Here's my dockerfile. Notice that I'm building on top of a slightly different image:
...
``` bash
docker build -t test-queue --build-arg user=djnavarro --build-arg repo=queue .
```
...
``` bash
docker build -t test-praise --build-arg user=rladies --build-arg repo=praise .
```
...
``` bash
docker build -t test-ggplot2 --build-arg user=tidyverse --build-arg repo=ggplot2 .
```
...
It's nice that I didn't have to rebuild my testing environment multiple times thanks to docker caching the layers, but it's not the most elegant solution. Given that the testing environment is one that I'll have to use with pretty much any package, it makes more sense to host that as its own image. 
