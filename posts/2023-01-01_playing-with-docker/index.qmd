...
Sometimes the easiest way to tell a story is to begin at the ending, and -- spoiler! -- I did in fact succeed in my attempt,^[Set your sights low enough and it is very easy to achieve your goals.] and I am now the proud^[No. Just no.] maintainer of two hastily-constructed images hosted [on the github container repository](https://github.com/djnavarro/arch-r/pkgs/container/arch-r). Now that I have these things, it should be really easy for us to put together a simple project that will run R code using these images and -- even though I'm going to be using my ubuntu laptop -- have it be executed by a container that is running arch. 

Oh. The. Thrill.

Be. Still. My. Beating. Heart.

Okay, so here it is. Accompanying this post is a project called [`system-check`](https://github.com/djnavarro/quarto-blog/tree/main/posts/2023-01-01_playing-with-docker/system-check) that consists of a three-line dockerfile and a two-line R script. Here is the complete source code for both of them:
...
Let's ignore the dockerfile for a moment and focus on the R code. Setting aside all the code that makes the output pretty, all it's doing is printing `osVersion` and calling `Sys.getlocale()`. Here's what happens when I run the script on my ubuntu laptop, without using docker in any way:
...
The first part of the output tells me my operating system (ubuntu), and the second part specifies the locale. I'm in Australia so for most things my locale is `en_AU.UTF-8`.

All of this should be quite different for an arch user in the United States. The docker image that I built -- which goes by the quaint and oh-so-memorable name `ghcr.io/djnavarro/arch-r-base` -- simulates exactly that. The computing environment runs arch and the system locale is set to `en_US.UTF-8`. In other words, this is a script that should produce quite different output if it's executed inside a container running the `arch-r-base`^[It's an open question how long I'm going to last in this post before making an Archer joke.] image, right? 

So here's our dockerfile again: 

```{dockerfile system-check-dockerfile-1}
#| eval: false
#| filename: "./system-check/Dockerfile"
FROM ghcr.io/djnavarro/arch-r-base:release
COPY script.R /home/script.R
...
It's a sequence of three **docker instructions**. Like (almost)^[Technically it's possible for an `ARG` instruction to precede a `FROM` instruction but I'm yet to actually see that in the wild.] every dockerfile, it begins with a `FROM`^[Much like SQL clauses, docker instructions are written in uppercase by convention. They don't actually *have* to be uppercase, but again, I've never seen a dockerfile written any other way. Along the same lines, your dockerfile doesn't actually have to be called "Dockerfile", but it's the default and everyone uses it.] instruction that specifies the name of a preexisting docker image to use as a starting point. 
...
The "already exists" part of the output is worth commenting on.^[I swear I had something for this.] I'd been playing around with some of the images upon which `ghcr.io/djnavarro/arch-r-base:release` is built earlier (e.g., it's built on an arch linux image that I already have on my laptop) and docker is smart enough to realise it only has to download and build some layers. That's a really nice feature. Among other things, it means that if I try building this image a second time, docker can use the cached copies of the build steps because nothing has changed and it doesn't have to do anything:
...
Notice that I'm building on top of a slightly different image. The `system-check` image starts by building from `ghcr.io/djnavarro/arch-r-base:release`, whereas `test-on-check` uses `ghcr.io/djnavarro/arch-r-test:release`. The two are related: `arch-r-test:release` takes `arch-r-base:release` as its starting point and adds some system and R packages that are super-handy when building an R package and running its unit tests. 

```{r whale-2}
#| echo: false
#| column: margin
#| fig-cap: I need a different plot here
whale(2)
```
...
#| echo: false
#| column: margin
#| fig-cap: Again, I need something more interesting than this
whale(3)
```
