...
sample_whales3 <- function(seed = NULL, nrow = 4, ncol = 6) {
  ...
  shades <- sample(
    x = ggthemes::canva_palettes$`Cool blues`[-1],
    size = nrow * ncol,
    replace = TRUE
  )
  ...
  ggplot(mapping = aes(x, y, fill = look, colour = look)) +
    ...
    theme(
      ...
      plot.margin = unit(rep(20, 4), "mm"),
      panel.border = element_rect(fill = NA, colour = "white", linewidth = 1)
    )
}

pic <- sample_whales3(seed = 2, 4, 4)
file <- here::here("posts/2023-01-01_playing-with-docker/whales.png")
bg <- ggthemes::canva_palettes$`Cool blues`[[1]]
ggsave(file, pic, width = 10, height = 10, bg = bg, dpi = 150)
