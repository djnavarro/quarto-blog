...
In that sense, assertr is complementary to the other three approaches. For example, you could use assertr to check the `data` input to a function that takes a data frame as the primary argument, but then use assertthat to test the others. 

Let's start with some data:

```{r}
#| label: archibald data
archibald <- readr::read_csv("archibald.csv", show_col_types = FALSE)
archibald
```
...
Here's an example where the validation checks pass and the analysis goes forward:
...
```{r}
#| label: assertr example 2
#| error: true
not_missing <- \(x) !is.na(x)
