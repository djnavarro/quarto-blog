...
In essence, that's how the magic works here. It's a very clever trick, and I imagine it's something that a lot of data analysts find super handy... but not something I'd want to do when writing package code. When calling mrgsolve in a more programmatic way I'd probably write code that looks more like this:

```{r}
mod <- mread(model = "pk1", project = modlib_dir, quiet = TRUE)
events <- ev(amt = 100, ii = 24, addl = 9)
...
Okay, now that we've resolved that question, let's return to the business of running simulations?
...
Again, this is not technically a data frame, but it's easily coercible to one, so of that's what you want you can do this:
