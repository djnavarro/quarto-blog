...
```{r}
mod <- mread(model = "pk1", project = fs::path_package("mrgsolve", "models"))
```
...
It's a clever trick, and I imagine it's something that a lot of data analysts find super handy. That said, it's probably not something I would use myself. I'm a simple girl who likes her coffee black and her functions boring, so if I were inclined to use `ev()` in my workflow at all, I'd be more keen on code that looks like this:

```{r}
mod <- mread_file(
  file = "pk1.cpp", 
  project = fs::path_package("mrgsolve", "models"), 
  quiet = TRUE
)
...
The output here looks the same, but this time the output is a regular data frame, and the command to run a simulation that *requires* a data frame as input might look like this:

```{r}
mrgsim_d(
  x = mod,
  data = ev_expand(amt = 100, ii = 24, addl = 9),
  end = 480,
  delta = 0.1
)
```
...
We're almost done unpacking the simple example, but I want to rewrite the code one last time. Previously, when I called `mrgsim_d()` I passed arguments `end` and `delta` as a way to override the default assumptions about what time points we would use when running our simulations. Internally, these arguments are used to construct a "tgrid" object. We can construct this object explicitly by calling `tgrid()`:
...
Taking a step back, it's worth thinking a little about how we want to write our simulation code. The piped code I started with probably works nicely for some people, but it's not my preferred way to do this. The way I think of these things, a simulation has *three* main inputs (model object, event schedule, simulation times), and I find the code easier to read when these three inputs are passed as three separate arguments. After a little refactoring, this...
...
It's quite a bit more verbose than the original code, but -- speaking personally here -- the second version makes more sense to me.  
