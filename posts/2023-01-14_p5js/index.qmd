...
```{ojs}
class Dot {
  constructor(sketch, x, y, colour) {
    this.s = sketch;
    this.x = x | 0;
    this.y = y | 0;
    this.colour = colour;
    this.size = 1;
    this.freq = this.s.random(-1, 1);
    this.velX = this.s.random(-1, 1);
    this.velY = this.s.random(-1, 1);
    this.phase = this.s.random(0, this.s.PI)
  }

  on() {
    this.s.noStroke();
    this.s.fill(this.colour);
    this.s.ellipse(this.x, this.y, this.size, this.size);
  }
  
  oscillate() {
    let theta = this.s.frameCount / 100 * this.freq;
    this.x += this.s.sin(theta + this.phase);
    this.y += this.s.cos(theta + this.phase);
  }
  
  move() {
    this.x += this.velX;
    this.y += this.velY;
    if (this.x + this.size < 0 || this.x > this.s.width - this.size) {
      this.velX = -this.velX;
    }
    if (this.y + this.size < 0 || this.y > this.s.height - this.size) {
      this.velY = -this.velY;
    }
  }
}
