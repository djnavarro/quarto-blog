...
Okay, that's helpful. We can see what all the columns are and what kind of data they contain. I'm still pretty new to data science workflows in Python, but it's not too difficult to do a little bit of data wrangling with Pandas. For instance, we can take a look at the distribution of nationalities among published authors. The table below counts the number of distinct publications (Trove IDs) and authors for each nationality represented in the data:
